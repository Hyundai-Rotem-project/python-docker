...
# (1) 일단 현재 카메라 화면을 가져오는 함수 (가짜로)
def get_current_camera_frame():
    try:
        # ★ capture.jpg를 불러오는 mock 버전
        image = Image.open('capture.jpg').convert('RGB')
        return image
    except Exception as e:
        print(f"🚫 Failed to get camera frame: {e}")
        return None
...
async def analyze_obstacle(obstacle, index):
    ...
    if frame is None:
        return {"className": "unknown", "position": (x_center, z_center), "confidence": 0.0}

    try:
        results = model.predict(frame, verbose=False)
        detections = results[0].boxes.data.cpu().numpy() if results and hasattr(results[0], 'boxes') else np.array([])
        ...
        for det in detections:
            ...
            bbox_center_z = (y1 + y2) / 2  # ★ 여기서는 그냥 y좌표를 z로 치환 (가짜 변환)

            # 거리 계산 (★ 실제 world<->pixel 변환 없이 간단히 비교)
            distance = math.hypot(x_center - bbox_center_x, z_center - bbox_center_z)
        ...
        if best_detection is not None:
            ...
            print(f"🧩 장애물 ({x_center:.2f}, {z_center:.2f}) -> 탐지: {class_name} (conf={confidence:.2f})")
        else:
            class_name = "unknown"
            confidence = 0.0
            print(f"🧩 장애물 ({x_center:.2f}, {z_center:.2f}) -> 탐지 실패")

    except Exception as e:
        print(f"🚫 analyze_obstacle error: {e}")
        class_name = 'unknown'
        confidence = 0.0
...
async def analyze_obstacle(obstacle, index):
    x_center = (obstacle["x_min"] + obstacle["x_max"]) / 2
    z_center = (obstacle["z_min"] + obstacle["z_max"]) / 2
    image_data = obstacle.get("image") ## 이미지 데이터가 포함된 경우 
...
def detect():
    ...
    print(image)
    if not image:
        print("🚫 YOLO: No image received in /detect")
        logging.error("YOLO: No image /detect")
    ...
    try:
        ...
        return jsonify({'detections': filtered_results})
    
    except Exception as e:
        print(f"🔍 YOLO failed: {e}")
        logging.error(f"YOLO failed: {e}", exec_info =True)
