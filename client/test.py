...
class Grid:
    ...
    def set_obstacle(self, x_min, x_max, z_min, z_max, start_pos=None, goal_pos=None, buffer=1.0):
        # 1m 버퍼 적용 (그리드 단위로 1m 추가)
        x_min = max(0, min(int(round(x_min - buffer)), self.width - 1))
        x_max = max(0, min(int(round(x_max + buffer)), self.width - 1))
        z_min = max(0, min(int(round(z_min - buffer)), self.height - 1))
        z_max = max(0, min(int(round(z_max + buffer)), self.height - 1))
...
CONFIDENCE_THRESHOLD = 0.2  # 낮춤
TRAPPED_TIMEOUT = 1.0
ESCAPE_ROTATION_ANGLE = 90.0
SHOTS_PER_ENEMY = 1
SHOT_INTERVAL = 1.0
ENEMY_PAUSE_DURATION = 3.0  # 적 감지 시 대기 시간
...
def draw_crosshair_box(img, bbox, label, class_name):
        ...
        for idx, obstacle in enumerate(obstacles):
            ...
            if obs_distance < DETECTION_RANGE:
                ...
                if detection["className"] in ENEMY_CLASSES and detection["confidence"] >= CONFIDENCE_THRESHOLD:
                    ...
                    print(f"🔫 Enemy detected at {obs_center}, distance={obs_distance:.2f}m, stopping for {ENEMY_PAUSE_DURATION}s")
                    logging.info(f"Enemy detected at {obs_center}, distance={obs_distance:.2f}m, stopping for {ENEMY_PAUSE_DURATION}s")
                    ...
                    print(f"🚗 Resuming MOVING after firing at {obs_center}")
                    logging.info(f"Resuming MOVING after firing at {obs_center}")
                    break
...
def detect():
    ...
    if not image:
        logging.error("No image received in request")
        return jsonify({"error": "No image received"}), 400
    
    logging.info(f"Image received: filename={image.filename}, content_type={image.content_type}")
    
    try:
        pil_image = Image.open(image)
        logging.info(f"PIL image loaded: format={pil_image.format}, size={pil_image.size}, mode={pil_image.mode}")
        pil_image.save("input_image.jpg")
        logging.info("Saved input image to input_image.jpg")
        ...
        detections = results[0].boxes.data.cpu().numpy()
        logging.info(f"YOLO raw detections: shape={detections.shape}, content={detections.tolist()}")
        ...
        for box in detections:
            ...
            logging.info(f"Box: class_id={class_id}, confidence={confidence:.2f}, bbox={box[:4].tolist()}")
            if class_id not in target_classes:
                logging.warning(f"Skipping invalid class ID: {class_id}")
                continue
            class_name = target_classes[class_id]
            if confidence < CONFIDENCE_THRESHOLD:
                logging.info(f"Skipping low confidence: {class_name}, confidence={confidence:.2f}")
                continue
            bbox = [float(coord) for coord in box[:4]]
            width = bbox[2] - bbox[0]
            height = bbox[3] - bbox[1]
            if class_name == 'wall2' and width * height < 1000:
                logging.info(f"Small wall2 ignored: width={width}, height={height}")
                continue
            label = f"{class_name} {confidence:.2f}"
            img = draw_crosshair_box(img, bbox, label, class_name)
        ...
        logging.info(f"Filtered detections: {len(filtered_results)}")
        img.save('detected_image.jpg')
        logging.info("Saved visualized image to detected_image.jpg")
        
        buffered = BytesIO()
        img.save(buffered, format="PNG")
        image_base64 = base64.b64encode(buffered.getvalue()).decode('utf-8')
        logging.info(f"Visualized image Base64 length: {len(image_base64)}")
        ...
        logging.error(f"YOLO detection failed: {str(e)}", exc_info=True)
...
def info():
        ...
        if obs_distance < DETECTION_RANGE:
            ...
            if detection["className"] in ENEMY_CLASSES and detection["confidence"] >= CONFIDENCE_THRESHOLD:
                ...
                with state_lock:
                    player_state["state"] = "STOPPED"
                    player_state["pause_start_time"] = current_time
                print(f"🔫 Enemy detected at {obs_center}, stopping for {ENEMY_PAUSE_DURATION}s")
                logging.info(f"Enemy detected at {obs_center}, stopping for {ENEMY_PAUSE_DURATION}s")
                asyncio.run(asyncio.sleep(ENEMY_PAUSE_DURATION))
                asyncio.run(shoot_at_target(obs_center))
                with state_lock:
                    player_state["state"] = "MOVING"
                    player_state["pause_start_time"] = None
                print(f"🚗 Resuming MOVING after firing at {obs_center}")
    ...
    with state_lock:
        ...
        elif player_state["state"] == "MOVING" and not enemy_detected:
            dx, dz = player_state["destination"]
            px, pz = player_state["position"]
...
def update_obstacle():
    ...
    for idx, obs in enumerate(obstacles):
        logging.info(f"Obstacle {idx}: x_min={obs['x_min']}, x_max={obs['x_max']}, z_min={obs['z_min']}, z_max={obs['z_max']}, image_exists={obs['image'] is not None}")
