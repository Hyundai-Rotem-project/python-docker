...
def detect():
    ...
    model_path = 'best_add.pt'

    Inference = RealTimeInference(model_path=model_path)
    enemy_list = Inference.log2pred(left_dir,right_dir,log_path)
    enemy_list['list'] = sorted(enemy_list['list'], key=lambda item: item['distance'])
